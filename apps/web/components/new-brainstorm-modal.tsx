...
type NewBrainstormModalProps = {
  open: boolean;
  onClose: () => void;
  onCreate: (input: { title: string; description: string; slug: string }) => Promise<void>;
};

export function NewBrainstormModal({ open, onClose, onCreate }: NewBrainstormModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 dark:bg-black/50">
      <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-5 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">New brainstorm</h2>
        <p className="mb-4 text-sm text-zinc-500 dark:text-zinc-400">
          Start an exploratory workspace for concepts and research.
        </p>
        ...
        <form
          ...
          onSubmit={async (event) => {
            ...
            try {
              ...
              setError(
                createError instanceof Error ? createError.message : "Failed to create brainstorm"
              );
            } finally {
              setSaving(false);
            }
        ...
        >
          <div className="space-y-1">
            <Label htmlFor="brainstorm-title">Title</Label>
            <Input
              id="brainstorm-title"
              value={title}
          ...
          <div className="space-y-1">
            <Label htmlFor="brainstorm-slug">Slug</Label>
            <Input
              id="brainstorm-slug"
              value={slug}
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              Preview: /{slugPreview || "your-brainstorm-slug"}
            </p>
          </div>

          <div className="space-y-1">
            <Label htmlFor="brainstorm-description">Description</Label>
            <textarea
